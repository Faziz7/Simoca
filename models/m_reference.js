...
function renderPageError(req,res, sess, message, url ='/reference/pemilik_kapal/'){
  hbs.registerPartial('content', fs.readFileSync( './views/error_page/505Page.html', 'utf8'));
  res.render('main', {nama_orang : sess['nama_pj'], role_user : sess['role_user'], message : message, url : url})
}
...
//===================================================== pelabuan ======================================================
exports.getDetailPelabuhan = async function(req, res){
  let id_pelabuhan = req.body.id_pelabuhan
  conn.query("SELECT * FROM tr_pelabuhan WHERE id_pelabuhan = ?", [id_pelabuhan], function(err, rows, fields){
      if(err){
...
exports.updatePelabuhan = async function(req,res,sess){
  var data_referensi = req.body
  conn.query(`UPDATE tr_pelabuhan SET latitude = ?, longitude = ?, kd_pelabuhan = ?, nama_pelabuhan = ?, nama_kabkota = ?,alamat_pelabuhan = ?, telp_pelabuhan = ?, website = ?, fasilitas = ? WHERE id_pelabuhan = ?`, 
    [data_referensi.latitude, data_referensi.longitude, data_referensi.kd_pelabuhan, data_referensi.nama_pelabuhan, 
    data_referensi.nama_kab_kota, 
    data_referensi.alamat_pelabuhan, data_referensi.telp_pelabuhan, data_referensi.website, data_referensi.fasilitas,
    data_referensi.id_pelabuhan], 
    function(err2, rows2, fields2) {
      if(err2){
        console.log(err2)
        renderPageError(req,res,sess,'Mohon maaf, Gagal Update data Referensi','/reference/pelabuhan/')
      }else{
        res.redirect('/reference/pelabuhan/')
      }
    })
...
exports.deletePelabuhan = async function(req,res,sess){
  var data_referensi = req.body
  console.log(data_referensi.id_pelabuhan)
...
//===================================================== Pemilik Kapal =================================================
exports.getListPemilik_kapal = async function(callback,sess){
  if(sess['role_user'] == '01'){
    let sql = "SELECT *, case when status = '00' then 'Belum Disetujui' when status ='01' then 'Disetujui' else 'Ditolak' end as status FROM tr_pemilik_kapal a join tb_user b on a.id_user = b.id_user join tb_data_kapal c on a.id_kapal = c.id_kapal";
    conn.query(sql, (err, results) => {
      if(err) throw callback(err,null);
        callback(null,results);
    })
  }else{
    let sql = "SELECT *,case when status = '00' then 'Belum Disetujui' when status ='01' then 'Disetujui' else 'Ditolak' end as status FROM tr_pemilik_kapal a join tb_user b on a.id_user = b.id_user join tb_data_kapal c on a.id_kapal = c.id_kapal where b.id_user = '"+sess['id_user']+"' and status != '02'";
    conn.query(sql, (err, results) => {
      if(err) throw callback(err,null);
        callback(null,results);
    })
...
exports.addPemilik_kapal = async function(req,res,sess){
  var data_referensi = req.body
  await conn.query("SELECT * FROM tb_data_kapal_detail where IMO_MMSI like ? and NO_TANDA_DAFTAR like ? and NO_AKTA like ?", ['%'+data_referensi.mmsi+'%','%'+data_referensi.no_tanda_daftar+'%','%'+data_referensi.no_akta+'%'], function(err, rows, fields){
    if(err){
...
exports.updateStatusPemilik = async function(req, res, sess){
  var data_referensi = req.body
  console.log(req.body)
  conn.query("UPDATE tr_pemilik_kapal set status = ? WHERE id_pemilik_kapal = ?", 
    [data_referensi.status, data_referensi.id_pemilik_kapal],
    function(err2, rows2, fields2) {
      if(err2){
        renderPageError(req,res,sess,'Mohon maaf, Gagal hapus data Referensi')
      }else{
