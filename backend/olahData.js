	...
	if(data_kapal.length === 0){
		...
		for(let i = 0; i < data_kapal.length; i++){
			...
			for(let j = 0; j < data_pelabuhan.length; j++){
				let point_a = {lat : data_pelabuhan[j]['longitude'], lng : data_pelabuhan[j]['latitude']}
				let dataHaversine = await haversine(point_b, point_a)
				tampung_jarak.push({mssi : data_kapal[i]['mmsi'] ,id_pelabuhan : data_pelabuhan[j]['id_pelabuhan'], jarak_terdekat : dataHaversine})
			}
			
			tampung_jarak = await tampung_jarak.sort(function(a,b){
				return a.jarak_terdekat - b.jarak_terdekat
			})		

			console.log(tampung_jarak +"<<<<<<<<<<<<<<KONTOL")
...
async function olahData_backup(){
	...
	for(let i = 0; i < data_kapal.length; i++){
        ...
        await updateFlagPenarikanDataKapal(data_kapal[i]['id_penarikan'])
        delete data_kapal[i]['id_penarikan']
		let point_a = {lat : data_pelabuhan[0]['longitude'], lng : data_pelabuhan[0]['latitude']}
		let point_b = {lat : data_kapal[i]['last_longitude'], lng : data_kapal[i]['last_latitude']}
